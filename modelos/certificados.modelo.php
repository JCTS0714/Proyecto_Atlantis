...
class ModeloCertificados{
  ...
  static public function mdlCrearCertificado($tabla, $datos){
    $sql = "INSERT INTO $tabla (nombre, ruc, usuario, clave, fecha_creacion, fecha_vencimiento, estado, observacion, creado_por, creado_en) VALUES (:nombre,:ruc,:usuario,:clave,:fecha_creacion,:fecha_vencimiento,:estado,:observacion,:creado_por,NOW())";
  ...
  static public function mdlEditarCertificado($tabla, $datos){
    $sets = [];
    $params = [];
    foreach(['nombre','ruc','usuario','clave','fecha_creacion','fecha_vencimiento','estado','observacion'] as $f){ if(isset($datos[$f])){ $sets[] = "$f = :$f"; $params[":$f"] = $datos[$f]; } }
