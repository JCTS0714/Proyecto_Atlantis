<?php
/**
 * Configuraci├│n de rutas base para entornos locales/producci├│n.
 * Define BASE_URL que apunta al esquema + host + carpeta del script.
 */
// Determinar si estamos en un entorno local seg├║n variables de entorno
...
if ($appEnv === 'local' || $forceHttp == '1') {
	$protocol = 'http';
	// Priorizar configuraci├│n expl├¡cita en config/local.php (clave BASE_HOST)
	$host = $config['BASE_HOST'] ?? getenv('BASE_HOST') ?: (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'localhost');
} else {
	// Detectar protocolo (producci├│n)
...
// Path base (carpeta donde est├í index.php), ├║til para entornos locales
$scriptDir = rtrim(dirname($_SERVER['SCRIPT_NAME']), '\\/');

// Construir BASE_URL sin slash final (ej: http://localhost/Proyecto_Atlantis/Ventas)
define('BASE_URL', $protocol . '://' . $host . $scriptDir);

// Opcional: ruta absoluta del filesystem a la ra├¡z del proyecto
